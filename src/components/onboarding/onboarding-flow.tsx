    ...
    <main className="mx-auto flex min-h-dvh max-w-lg flex-col px-6 pb-8" style={{ paddingTop: "calc(var(--sat) + 2rem)" }}>
      ...
      {step === 0 ? (
        <section className="flex flex-1 flex-col justify-center gap-4">
          <h1 className="font-display text-[30px] font-extrabold tracking-[-0.5px]">Let&apos;s get you ready for Warwick</h1>
          ...
          <label className="flex flex-col gap-1.5 text-sm font-semibold">
            Your first name
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Arielle" />
          </label>
      ...
      {step === 1 ? (
        <section className="flex flex-1 flex-col gap-4 pt-8">
          <h1 className="font-display text-[28px] font-extrabold tracking-[-0.5px]">Which Warwick accommodation are you staying in?</h1>
      ...
      {step === 4 ? (
        <section className="flex flex-1 flex-col gap-4 pt-8">
          ...
          <div className="mt-auto flex gap-2"><Button variant="ghost" onClick={() => setStep(3)}>Back</Button><Button variant="ghost" className="flex-1" onClick={() => finish(null)} loading={saving}>Skip for now</Button></div>
        </section>
      ) : null}
    </main>
  );
